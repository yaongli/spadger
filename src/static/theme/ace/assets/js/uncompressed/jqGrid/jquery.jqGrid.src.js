...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		$("<div class='loading ui-parser-default ui-parser-active' id='load_"+this.id+"'>"+this.p.loadtext+"</div>").insertBefore(gv);
		...
		addRowNum = function (pos,irow,pG,rN) {
			...
			return "<td role=\"gridcell\" class=\"ui-parser-default jqgrid-rownum\" "+prp+">"+v+"</td>";
		},
		...
		constructTr = function(id, hide, altClass, rd, cur, selected) {
			var tabindex = '-1', restAttr = '', attrName, style = hide ? 'display:none;' : '',
				classes = 'ui-widget-content jqgrow ui-row-' + ts.p.direction + (altClass ? ' ' + altClass : '') + (selected ? ' ui-parser-highlight' : ''),
		...
		setPager = function (pgid, tp){
			...
			var sep = "<td class='ui-pg-button ui-parser-disabled' style='width:4px;'><span class='ui-separator'></span></td>",
			pginp = "",
		...
		sortData = function (index, idxcol,reload,sor, obj){
			...
			if(ts.p.multiSort) {
				...
				if(ts.p.frozenColumns) {
					ts.grid.fhDiv.find("span.ui-grid-ico-sort").addClass('ui-parser-disabled');
		...
		imgs = "<span class='s-ico' style='display:none'><span sort='asc' class='ui-grid-ico-sort ui-icon-asc"+iac+" ui-parser-disabled ui-icon ui-icon-triangle-1-n ui-sort-"+dir+"'></span>";
		imgs += "<span sort='desc' class='ui-grid-ico-sort ui-icon-desc"+idc+" ui-parser-disabled ui-icon ui-icon-triangle-1-s ui-sort-"+dir+"'></span></span>";
		if(ts.p.multiSort) {
		...
		for(i=0;i<this.p.colNames.length;i++){
			...
			thead += "<th id='"+ts.p.id+"_"+ts.p.colModel[i].name+"' role='columnheader' class='ui-parser-default ui-th-column ui-th-"+dir+"'"+ tooltip+">";
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if($(ptr).length === 0 || ptr[0].className.indexOf( 'ui-parser-disabled' ) > -1 || ($(td,ts).closest("table.ui-jqgrid-btable").attr('id') || '').replace("_frozen","") !== ts.id ) {
				return this;
			}
		...
		var arf = ts.p.hidegrid===true ? $("<a role='link' class='ui-jqgrid-titlebar-close ui-corner-all HeaderButton' />").hover(
			function(){ arf.addClass('ui-parser-hover');},
			function() {arf.removeClass('ui-parser-hover');})
		...
		if(ts.p.footerrow) {
			...
			if(ts.p.rownumbers) { grid.footers[0].className = 'ui-parser-default jqgrid-rownum'; }
...
$.jgrid.extend({
	...
	setSelection : function(selection,onsr, e) {
		return this.each(function(){
			...
			if(!pt || !pt.className || pt.className.indexOf( 'ui-parser-disabled' ) > -1 ) { return; }
			function scrGrid(iR){
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(ni){
						...
						row[row.length] = "<td role=\"gridcell\" class=\"ui-parser-default jqgrid-rownum\" "+prp+">0</td>";
					}
					if(gi) {
		...
		return this.each(function(){
			...
			buildRuleMenu = function( elem, left, top ){
				...
				for ( i = 0 ; i < options.sopt.length; i++) {
					...
					if(ina !== -1) {
						selclass = selected === p.odata[ina].oper ? "ui-parser-highlight" : "";
			...
			$.each($t.p.colModel,function(ci){
				...
				th = $("<th role='columnheader' class='ui-parser-default ui-th-column ui-th-"+$t.p.direction+"'></th>"),
			...
			for (i = 0; i < cml; i++) {
				...
				if (iCol >= 0) {
					...
					$colHeader = $('<th>').attr({role: "columnheader"})
						.addClass("ui-parser-default ui-th-column-header ui-th-"+ts.p.direction)
	...
	setFrozenColumns : function () {
		return this.each(function() {
			...
			if( maxfrozen>=0 && frozen) {
				...
				$t.grid.fhDiv = $('<div style="position:absolute;left:0px;top:'+top+'px;height:'+hth+'px;" class="frozen-div ui-parser-default ui-jqgrid-hdiv"></div>');
...
"use strict";	
	...
	$.fn.fmatter.actions = function(cellval,opts) {
		...
		if(op.editformbutton){
			ocl = "id='jEditButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'formedit'); onmouseover=jQuery(this).addClass('ui-parser-hover'); onmouseout=jQuery(this).removeClass('ui-parser-hover'); ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
		} else if(op.editbutton){
			ocl = "id='jEditButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'edit'); onmouseover=jQuery(this).addClass('ui-parser-hover'); onmouseout=jQuery(this).removeClass('ui-parser-hover') ";
			str += "<div title='"+$.jgrid.nav.edittitle+"' style='float:left;cursor:pointer;' class='ui-pg-div ui-inline-edit' "+ocl+"><span class='ui-icon ui-icon-pencil'></span></div>";
		}
		if(op.delbutton) {
			ocl = "id='jDeleteButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'del'); onmouseover=jQuery(this).addClass('ui-parser-hover'); onmouseout=jQuery(this).removeClass('ui-parser-hover'); ";
			str += "<div title='"+$.jgrid.nav.deltitle+"' style='float:left;margin-left:5px;' class='ui-pg-div ui-inline-del' "+ocl+"><span class='ui-icon ui-icon-trash'></span></div>";
		}
		ocl = "id='jSaveButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'save'); onmouseover=jQuery(this).addClass('ui-parser-hover'); onmouseout=jQuery(this).removeClass('ui-parser-hover'); ";
		str += "<div title='"+$.jgrid.edit.bSubmit+"' style='float:left;display:none' class='ui-pg-div ui-inline-save' "+ocl+"><span class='ui-icon ui-icon-disk'></span></div>";
		ocl = "id='jCancelButton_"+rowid+"' onclick=jQuery.fn.fmatter.rowactions.call(this,'cancel'); onmouseover=jQuery(this).addClass('ui-parser-hover'); onmouseout=jQuery(this).removeClass('ui-parser-hover'); ";
...
//Helper functions
	...
	createModal : function(aIDs, content, p, insertSelector, posSelector, appendsel, css) {
		...
		.hover(function(){ahr.addClass('ui-parser-hover');},
			function(){ahr.removeClass('ui-parser-hover');})
	...
	info_dialog : function(caption, content,c_b, modalopt) {
		...
		if(mopt.buttons.length > 0) {
			for(i=0;i<mopt.buttons.length;i++) {
				...
				buttstr += "<a id='"+mopt.buttons[i].id+"' class='fm-button ui-parser-default ui-corner-all'>"+mopt.buttons[i].text+"</a>";
			}
		}
		...
		cnt += c_b ? "<div class='ui-widget-content ui-helper-clearfix' style='text-align:"+mopt.buttonalign+";padding-bottom:0.8em;padding-top:0.5em;background-image: none;border-width: 1px 0 0 0;'><a id='closedialog' class='fm-button ui-parser-default ui-corner-all'>"+c_b+"</a>"+buttstr+"</div>" :
	...
	return this.each( function() {
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			if(parentgroup === null) {
				table.append("<tr class='error' style='display:none;'><th colspan='5' class='ui-parser-error' align='"+align+"'></th></tr>");
			}
		...
		this.createTableRowForRule = function(rule, group ) {
			...
			} //retain the parser of disabled text fields in case of null ops
		...
		this.hideError = function() {
			$("th.ui-parser-error", this).html("");
			$("tr.error", this).hide();
		};
		this.showError = function() {
			$("th.ui-parser-error", this).html(this.p.errmsg);
			$("tr.error", this).show();
		};
...
$.jgrid.extend({
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				bS  ="<a id='"+fid+"_search' class='fm-button ui-parser-default ui-corner-all fm-button-icon-right ui-reset'><span class='ui-icon ui-icon-search'></span>"+p.Find+"</a>",
				bC  ="<a id='"+fid+"_reset' class='fm-button ui-parser-default ui-corner-all fm-button-icon-left ui-search'><span class='ui-icon ui-icon-arrowreturnthick-1-w'></span>"+p.Reset+"</a>",
				bQ = "", tmpl="", colnm, found = false, bt, cmi=-1;
				if(p.showQuery) {
					bQ ="<a id='"+fid+"_query' class='fm-button ui-parser-default ui-corner-all fm-button-icon-left'><span class='ui-icon ui-icon-comment'></span>Query</a>";
				}
				if(!p.columns.length) {
				...
				showFilter($("#"+fid));
				$(".fm-button:not(.ui-parser-disabled)",fil).hover(
					function(){$(this).addClass('ui-state-hover');},
					function(){$(this).removeClass('ui-state-hover');}
				);
			}
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			$(frm).append(tbl);
			var flr = $("<tr id='FormError' style='display:none'><td class='ui-parser-error' colspan='"+(maxCols*2)+"'></td></tr>");
			flr[0].rp = 0;
			$(tbl).append(flr);
			//topinfo
			...
			// buttons at footer
			var bP = "<a id='"+bp+"' class='fm-button ui-parser-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",
			bN = "<a id='"+bn+"' class='fm-button ui-parser-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>",
			bS  ="<a id='sData' class='fm-button ui-parser-default ui-corner-all'>"+p.bSubmit+"</a>",
			bC  ="<a id='cData' class='fm-button ui-parser-default ui-corner-all'>"+p.bCancel+"</a>";
			...
			if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
				bS  ="<a id='sNew' class='fm-button ui-parser-default ui-corner-all' style='z-index:1002'>"+p.bYes+"</a>";
				bN  ="<a id='nNew' class='fm-button ui-parser-default ui-corner-all' style='z-index:1002'>"+p.bNo+"</a>";
				bC  ="<a id='cNew' class='fm-button ui-parser-default ui-corner-all' style='z-index:1002'>"+p.bExit+"</a>";
	...
	viewGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			bn = rtlb ? "pData" : "nData",
				// buttons at footer
			bP = "<a id='"+bp+"' class='fm-button ui-parser-default ui-corner-left'><span class='ui-icon ui-icon-triangle-1-w'></span></a>",
			bN = "<a id='"+bn+"' class='fm-button ui-parser-default ui-corner-right'><span class='ui-icon ui-icon-triangle-1-e'></span></a>",
			bC  ="<a id='cData' class='fm-button ui-parser-default ui-corner-all'>"+p.bClose+"</a>";
			if(maxRows >  0) {
				var sd=[];
				$.each($(tbl)[0].rows,function(i,r){
					sd[i] = r;
			...
			$(".fm-button:not(.ui-parser-disabled)","#"+frmtb+"_2").hover(
				function(){$(this).addClass('ui-state-hover');},
				function(){$(this).removeClass('ui-state-hover');}
			);
			focusaref();
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				// error data
				tbl += "<tr id='DelError' style='display:none'><td class='ui-parser-error'></td></tr>";
				tbl += "<tr id='DelData' style='display:none'><td >"+rowids+"</td></tr>";
				tbl += "<tr><td class=\"delmsg\" style=\"white-space:pre;\">"+rp_ge[$t.p.id].msg+"</td></tr><tr><td >&#160;</td></tr>";
				// buttons at footer
				tbl += "</tbody></table></div>";
				var bS  = "<a id='dData' class='fm-button ui-parser-default ui-corner-all'>"+p.bSubmit+"</a>",
				bC  = "<a id='eData' class='fm-button ui-parser-default ui-corner-all'>"+p.bCancel+"</a>";
	...
	navGrid : function (elem, o, pEdit,pAdd,pDel,pSearch, pView) {
		...
		return this.each(function() {
			...
			for(i = 0; i<clone; i++) {
				...
				sep = "<td class='ui-pg-button ui-parser-disabled' style='width:4px;'><span class='ui-separator'></span></td>",
				pgid, elemids;
				if(i===0) {
					pgid = elem;
					elemids = $t.p.id;
	...
	navSeparatorAdd:function (elem,p) {
		...
		return this.each(function() {
			...
			if(findnav) {
				var sep = "<td class='ui-pg-button ui-parser-disabled' style='width:4px;'><span class='"+p.sepclass+"'></span>"+p.sepcontent+"</td>";
				if (p.position === 'first') {
					if (findnav.rows[0].cells.length === 0) {
				...
				$.ajax($.extend({
					...
					error:function(res,stat,err){
						...
						if($.isFunction(o.errorfunc) ) {
							...
							try {
								$.jgrid.info_dialog($.jgrid.errors.errcap,'<div class="ui-parser-error">'+ rT +'</div>', $.jgrid.edit.bClose,{buttonalign:'right'});
							} catch(e) {
								alert(rT);
							}
						}
	...
	editCell : function (iRow,iCol, ed){
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				cc.addClass("edit-cell ui-parser-highlight");
				$($t.rows[iRow]).addClass("selected-row ui-state-hover");
	...
	return this.each(function(){
		...
		var subGridXml = function(sjxml, sbid){
			...
			for (i = 0; i<ts.p.subGridModel[0].name.length; i++) {
				tddiv = $("<th class='ui-parser-default ui-th-subgrid ui-th-column ui-th-"+ts.p.direction+"'></th>");
...
$.jgrid.extend({
			...
			var sortable_opts = {
				...
				"placeholder": {
					element: function(item) {
						...
						.addClass(item[0].className+" ui-sortable-placeholder ui-parser-highlight")
	...
	gridDnD : function(opts) {
		...
		opts = $.extend({
			"drag" : function (opts) {
				return $.extend({
					start : function (ev, ui) {
						...
						ui.helper.addClass("ui-parser-highlight");
			...
			"ondrop" : null,
			"drop_opts" : {
				"activeClass": "ui-parser-active",
				"hoverClass": "ui-parser-hover"
			},
			"drag_opts" : {
				"revert": "invalid",
				"helper": "clone",
